(function() {
  ...
  module.exports = function(data) {
    return through.obj(function(file, enc, cb) {
      var error, error1;
      if (!file) {
        this.emit('error', new gutil.PluginError(PLUGIN_NAME, 'Files can not be empty'));
      }
      if (file.isNull()) {
        this.push(file);
        return cb();
  ...
  rewriteMeta = function(file, data) {
    var chunk1_offset, chunk2_offset, dirname, extname, isPreset, magic, metadata_offset, new_chunk1_offset, new_chunk2_offset, new_metadata, reader, writer;
  ...
  parseMetadata = function(file, reader) {
    var extname, hasNext, i, key, ret, size, value, valueType;
    extname = path.extname(file.path);
    ret = {
      file: file.path,
  ...
  replaceMetadata = function(reader, writer, data) {
    var hasNext, i, key, new_metadata, size, value, valueType, valye;
    new_metadata = {};
    hasNext = reader.readInt32();
    while (hasNext === 1) {
      key = reader.readString();
      ...
      switch (valueType) {
        case $.valueType.string:
          if (indexOf.call(_.keys(data), key) >= 0) {
            writer.push(reader.mark());
            value = data[key];
            if (key === 'tags') {
  ...
  replacePresetChunk1 = function(reader, writer, data) {
    ...
    while (itemId !== $.endOfMeta) {
      ...
      results.push(itemId = reader.readInt32());
    }
    return results;
  };
